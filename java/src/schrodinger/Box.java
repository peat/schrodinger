...
public class Box<T> {
  ...
  public static <T> Box apply( Change<T> ba, Box<T> b ) {
    if (b.isEmpty()) {
      return new Box<T>();
    } else {
  ...
  public static <T> Box apply( Combine<T> bc, List<Box<T>> boxes ) {
    ...
    for (Box<T> b : boxes) {
      ...
      T bVal = b.unpack();
      resultBox = new Box<T>( bc.apply( aVal, bVal ) ); 
    }
  ...
  public static <T> List apply( Filter<T> bf, List<Box<T>> boxes ) {
